...
class TestSpecifiedGraphCreator:
    ...
    def build_graphs(cls):
        ...
        random.seed(seed)
        np.random.seed(seed)
        cls.g1 = cfpq_data.BarabasiAlbertGraphCreator(100, 1, seed=seed, edge_labels="A").create()
        cls.g2 = cfpq_data.BinomialGraphCreator(42, 0.73, seed=seed, edge_labels="A").create()
    ...
    def test_g1(self):
        ...
        number_of_a = sum([1 for u, v, labels in self.g1.edges(data=True) if labels["label"] == "A"])
        number_of_b = sum([1 for u, v, labels in gin.edges(data=True) if labels["label"] == "B"])
    ...
    def test_g2(self):
        ...
        number_of_a = sum([1 for u, v, labels in self.g2.edges(data=True) if labels["label"] == "A"])
        number_of_b = sum([1 for u, v, labels in gin.edges(data=True) if labels["label"] == "B"])

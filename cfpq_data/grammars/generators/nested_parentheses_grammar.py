...
__all__ = ["nested_parentheses_grammar", "nested_parentheses_grammar_from_graph"]
...
def nested_parentheses_grammar_from_graph(
    ...
    *,
    ...
    """Returns a Nested Parentheses grammar that generates a language of the strings with nested parentheses
    corresponding to the most frequent edge labels of the given graph.
    ...
    n_types : int
        The number of edge labels to be treated as open parentheses.
    ...
    >>> cfg = nested_parentheses_grammar_from_graph(g, n_types=2)
    >>> cfg_to_text(cfg)
    'S -> \\nS -> label S label_r\\nS -> type S type_r'
    ...
    if len(sorted_labels) > n_types:
        sorted_labels = sorted_labels[:n_types]

    types = [(x, x + "_r") for x in sorted_labels]

    cfg = nested_parentheses_grammar(types, eps=eps, start_symbol=start_symbol)

    logging.info(f"Create a Nested Parentheses {cfg=} for {graph=}, {n_types=}, {eps=}")
